...
\section*{Houdini Message Receiver}

\begin{tikzpicture}[node distance=2cm]

\node (start) [startstop] {Message Client started};
\node (ioLoop) [process, below of=start] {Socket Message Loop};
...
\node (call) [process, below of=parse] {Call handling method};
...
\node (stop) [startstop, xshift=3cm, right of=fail] {Stop loop};
